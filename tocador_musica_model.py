...
class Model:
    ...
    def favoritar_musica(self):
        ...
        self.musica = self.controller.get_lista_reproducao()
        # Favoritar uma música selecionada na lista
        # self.cnx = mysql.connector.connect(user='root',password='', host='127.0.0.1', database='Favoritos')
    ...
    def favoritar_varias_musicas(self):
        # self.cnx = mysql.connector.connect(user='root',password='', host='127.0.0.1', database='Favoritos')
        self.cursor = self.cnx.cursor()
        self.musicas = [self.controller.get_musicas_selecionadas()]
